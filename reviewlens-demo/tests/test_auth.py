...
def test_validate_token_valid():
    user_id = auth.validate_token("fp_user_001_dev-secret-change-in-production")
    assert user_id == "user_001"


def test_validate_token_wrong_secret():
    user_id = auth.validate_token("fp_user_001_wrong-secret")
...
def test_validate_token_malformed():
    ...
    assert auth.validate_token("fp_only_two") is None
...
def test_require_auth_valid_token(app_context):
    with patch("auth.get_token_from_request", return_value="fp_user_001_dev-secret-change-in-production"):
...
def test_require_auth_invalid_token(app_context):
    with patch("auth.get_token_from_request", return_value="fp_user_001_bad-secret"):
